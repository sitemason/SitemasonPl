...
=head2 B<is_already_running>
 ...
 if (is_already_running) { $self->{io}->is_person && $self->{debug}->warning("Already running"); exit; }
...
=head2 B<get_answer>
 ...
 defined(my $answer = $self->{io}->get_answer) || return;
 defined(my $answer = $self->{io}->get_answer($array_of_inputs) || return;
...
=head2 B<prompt>
 ...
 defined(my $answer = $self->{io}->prompt($label, $preinputs, $regex, $invalid_message)) || exit;
...
Returns an answer from an array of choices:
 ...
 my $value = $self->{io}->get_menu_answer($choices_as_array, $label) || exit;
 my $value = $self->{io}->get_menu_answer($choices_as_array, $label, {
 	key_name	=> $key_name,	# optional, but requires value_name
 	value_name	=> $value_name,	# optional, but requires key_name
 	inputs		=> $inputs		# optional
 }) || exit;
 my $hash = $self->{io}->get_menu_answer($choices_as_arrayhash, $label, {
 	key_name	=> $key_name,
 	inputs		=> $inputs		# optional
 }) || exit;
...
=head2 B<pause>
 ...
 $self->{io}->pause($seconds_to_pause);
 $self->{io}->pause($seconds_to_pause, $label);
...
Pass 'end' to cleanly stop the progress bar.
 ...
 $self->{io}->progress($fraction_of_progress, [$label]);
...
sub get_term_color {
	# my $color_code = $io->get_term_color($color_name);
	my $self = shift || return;
	my $name = shift;
...
=head1 CHANGES
  ...
  2019-09-25 TJM - v9.0 Renamed to IO to go beyond the command line
